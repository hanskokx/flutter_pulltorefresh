...
void main() {
  testWidgets("from bottom pull up release gesture to load more", (
      ...
      Directionality(
        ...
        child: SmartRefresher(
          ...
          child: ListView.builder(
            itemBuilder: (c, i) => Center(child: Text(data[i])),
            itemCount: 20,
            itemExtent: 100,
          ),
        ),
    ...
    refreshController.position!.jumpTo(
      refreshController.position!.maxScrollExtent - 30,
    );
  ...
  testWidgets("strick to check tigger judge", (tester) async {
    ...
    refreshController.position!.jumpTo(
      refreshController.position!.maxScrollExtent - 216,
    );
  ...
  testWidgets("enableBallsticLoad=false test", (tester) async {
    ...
    //fling to bottom
    refreshController.position!.jumpTo(
      refreshController.position!.maxScrollExtent - 500,
    );
  ...
  group("check if the loading more times stiuation exists", () {
    testWidgets("loading->idle", (tester) async {
      ...
      refreshController.position!.jumpTo(
        refreshController.position!.maxScrollExtent - 100,
      );
