  ...
  testWidgets(
    ...
    (tester) async {
      final List<CustomFooter> footerData = [
        CustomFooter(
          builder: (_, c) => const SizedBox(height: 60.0, width: 60.0),
      ...
      for (final CustomFooter footer in footerData) {
        ...
        RenderSliverSingleBoxAdapter sliver = tester.renderObject(
          find.byType(SliverLoading),
        );
        // behind the bottom ,if else ,it is render error
        expect(
          sliver.child!.localToGlobal(const Offset(0.0, 0.0)),
          const Offset(0, 600),
        );
