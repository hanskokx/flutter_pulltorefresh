...
void main() {
  group("in Android ClampingScrollPhysics", () {
      ...
      (tester) async {
        ...
        while (tester.binding.transientCallbackCount > 0) {
          expect(
            ...
            greaterThanOrEqualTo(-250.0),
          );
          await tester.pump(const Duration(milliseconds: 20));
        }

        // from bottom flip up
        refreshController0.position!.jumpTo(
    ...
    testWidgets(
      "When clamping,enablePullDown = false,it shouldn't overscroll",
      (tester) async {
        final RefreshController refreshController0 = RefreshController();
        await tester.pumpWidget(
          MaterialApp(
  ...
  testWidgets("verity if refresh physics updated ", (tester) async {
    ...
    expect(
      (refreshController.position!.physics as RefreshPhysics).updateFlag,
      1,
    );
    await tester.pumpWidget(
      MaterialApp(
